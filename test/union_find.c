...
void test_new_union_find() {
    TEST_ASSERT_EQUAL_INT(0, testee.num_classes);
}

void test_add_subset() {
    int a = uf_add_subset(&testee);
    TEST_ASSERT_EQUAL_INT(1, testee.num_classes);
    TEST_ASSERT_EQUAL_INT(a, uf_find(a, &testee));

    int b = uf_add_subset(&testee);
    TEST_ASSERT_EQUAL_INT(2, testee.num_classes);
...
void test_union() {
    int a = uf_add_subset(&testee);
    int b = uf_add_subset(&testee);
    int c = uf_add_subset(&testee);

    TEST_ASSERT_EQUAL_INT(3, testee.num_classes);
...
void big_test() {
    for (int i = 0; i < 32; i++) {
        uf_add_subset(&testee);
    }

    TEST_ASSERT_EQUAL_INT(32, testee.num_classes);
    uf_union(1, 5, &testee);
